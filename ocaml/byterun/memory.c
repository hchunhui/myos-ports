...
int caml_page_table_initialize(mlsize_t bytesize)
{
  ...
  caml_page_table.entries = malloc(caml_page_table.size* sizeof(uintnat));
...
static int caml_page_table_resize(void)
{
  ...
  new_entries = malloc(2 * old.size* sizeof(uintnat));
  if (new_entries == NULL) {
...
static int caml_page_table_modify(uintnat page, int toclear, int toset)
{
  ...
  if (caml_page_table[i] == caml_page_table_empty) {
    unsigned char * new_tbl = malloc(Pagetable2_size* 1);
